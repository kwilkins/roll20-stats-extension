  ...
  for (const rollerName in rollParameters) {
    ...
    for (const diceRollTypeToAdd in diceRolls) {
      ...
      for (let i = 0; i < numberOfDiceRollTypeToAdd; i++) {
        messagesHtml.push(generateStandardDiceRollMessageFOO(DiceRollType[diceRollTypeToAdd], rollerName));
      }
    }
  }

  // TODO need to add non roll messages
  // TODO consider adding 'stacked' messages by the same roller where all sequential ones don't contain the .by element
...
const generateStandardDiceRollMessageFOO = (diceRollType: DiceRollType, rollerName: string): string => {
  ...
  const rollResult = Math.floor(Math.random() * +regexMatches[1]);
